...
def _build_brand_data(
    ...
    """Build the JSON-serializable data dict for a single brand."""
...
MULTI_OUTPUT_NAME = "mvp_dashboard_combined.html"


def _multi_brand_html(brands_data: dict[str, dict]) -> str:
    """Build a single HTML dashboard with a brand switcher for multiple case studies."""
    # Build embedded data scripts for each brand
    data_scripts = "\n".join(
        _json_script(f"presence-data-{case_id}", brand_data)
        for case_id, brand_data in brands_data.items()
    )

    # Build dropdown options
    options_html = "\n".join(
        f'  <option value="{case_id}">{brand_data["brand_name"]}</option>'
        for case_id, brand_data in brands_data.items()
    )

    first_case_id = next(iter(brands_data))
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Presence Rx - Multi-Brand Dashboard</title>
  <script src="https://cdn.plot.ly/plotly-2.35.2.min.js"></script>
  <style>
    :root {
      --peec-bg: #FFFFFF;
...
<div class="app-shell">
  ...
  <main class="workspace">
    <header class="topbar">
      <div class="page-kicker"><span class="nav-dot"></span>Presence Rx MVP dashboard</div>
      <div class="header-actions">
        <select id="brandSwitcher" class="brand-switcher-select" aria-label="Switch brand case study">
"""
        + options_html
        + """
        </select>
...
"""
        ...
        + """
<script>
// --- Load all brand datasets from embedded scripts ---
const brandDataSets = {};
document.querySelectorAll('script[id^="presence-data-"]').forEach(el => {
  const caseId = el.id.replace('presence-data-', '');
  brandDataSets[caseId] = JSON.parse(el.textContent);
});
...
// --- Brand-dependent rendering ---

function loadBrand(caseId) {
  currentCaseId = caseId;
  const data = brandDataSets[caseId];
  if (!data) return;
  ...
  function clusterOwner(row, owner) {
    if (owner?.ownership_status === 'target_owned' || (!row.gap_type && (row.visibility_target_share || 0) >= 0.5)) {
      return { name: brandName, label: `${brandName} owns`, tone: 'owner-target', visibility: row.visibility_target_share };
    }
    if (owner?.competitor_owner || row.visibility_competitor_owner) {
      const name = owner.competitor_owner || row.visibility_competitor_owner;
      return { name, label: `${name} owns`, tone: owner?.ownership_status === 'contested' ? 'watch' : 'owner-competitor', visibility: owner.competitor_visibility_share };
    }
    return { name: 'Unclear owner', label: 'Unclear owner', tone: 'unknown', visibility: owner?.competitor_visibility_share };
  }
  ...
  const rowsByGap = {};
  studyRows.forEach(row => {
    if (row.gap_type) { (rowsByGap[row.gap_type] = rowsByGap[row.gap_type] || []).push(row); }
  });
  document.getElementById('actionBriefCards').innerHTML = GAP_TYPE_ORDER.map(gapType => {
    const gapRows = rowsByGap[gapType] || [];
    if (!gapRows.length) return '';
  ...
  const prescriptionRows = prescription ? [
    ...(prescription.planned_topics || []).map(item => ({
      type: 'topic', operation_id: item.operation_id, name: item.name, gap_type: item.gap_type,
      country_code: '-', status: item.execution_status, source_cluster_id: item.source_cluster_id
    })),
    ...(prescription.planned_tags || []).map(item => ({
      type: 'tag', operation_id: item.operation_id, name: item.name, gap_type: item.kind,
      country_code: '-', status: item.execution_status, source_cluster_id: '-'
    })),
    ...(prescription.planned_prompts || []).map(item => ({
      type: 'prompt', operation_id: item.operation_id, name: item.text, gap_type: item.gap_type,
      country_code: item.country_code, status: item.execution_status, source_cluster_id: item.source_cluster_id
    }))
  ] : [];
  ...
  // --- Monitoring cards ---
  const prescriptionSummary = prescription?.summary || { planned_topics: 0, planned_tags: 0, planned_prompts: 0 };
  const promptGeos = prescription
    ? [...new Set((prescription.planned_prompts || []).map(item => item.country_code))].sort()
    : [];
...
def build_multi_brand_dashboard(brands_data: dict[str, dict]) -> str:
    """Build a single dashboard with brand switcher for all case studies.

    *brands_data* maps case_id -> data dict (same shape produced by
    ``_build_brand_data``).
    """
    return _multi_brand_html(brands_data)


def write_multi_brand_dashboard(content: str, out_dir: Path) -> Path:
    """Write the combined multi-brand dashboard HTML."""
    out_dir.mkdir(parents=True, exist_ok=True)
    path = out_dir / MULTI_OUTPUT_NAME
