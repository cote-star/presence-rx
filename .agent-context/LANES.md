...
| **Claude (design)** | Frontend / UX | Lovable dashboard surface (eligibility-critical) or Entire fallback. Webapp must follow Peec design philosophy (D-013) — design tokens already captured in `design.md`; apply Sections 9–11 directly. Also builds the static `/future-directions` page from FUTURE_DIRECTIONS.md (Tier 4 #21). Activated Sunday or on demand. | INDEX -> OVERVIEW -> [../docs/SCOPE_FINAL.md](../docs/SCOPE_FINAL.md) Tier 4 #20 + #21 -> [../docs/design.md](../docs/design.md) -> [../docs/FUTURE_DIRECTIONS.md](../docs/FUTURE_DIRECTIONS.md) -> [../docs/ARTIFACT_CONTRACTS.md](../docs/ARTIFACT_CONTRACTS.md) (study_ssot, hero_cards, pipeline_funnel) -> [../pm-dashboard.html](../pm-dashboard.html) for layout reference |
...
- **Parallel-safe modules:** Peec ingestion (Claude) / Gemini API (Gemini lane) / Tavily integration (any lane) / dashboard (Claude design or Lovable lane) — all independent until they read from the shared `study_ssot.json`. (Design-token capture from peec.ai is already done — see `design.md`.)
- **Serial-required:** Gap-type classifier (Tier 1.5) consumes Peec + Tavily + Gemini outputs and feeds the guardrail engine; build it after ingestion + enrichment, before guardrails. Guardrail engine must exist before artifact generation. Artifact generation must exist before dashboard wiring.
