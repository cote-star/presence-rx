...
- **Decision:** Steps 7-8 (channel activation, lift study, spend allocation) split into two assets: (a) a public-repo `FUTURE_DIRECTIONS.md` with concept-only content + a static page in the Lovable webapp, both clearly marked as illustrative and not calibrated against published benchmarks; (b) a private-only prototype using one-time-permissioned external benchmark data, built outside the public repo, demoed only in a live finalist pitch if advancing. Submission video may include a 5-15s sneak peek at the end using only conceptual visuals — no real benchmark data, no source names, no source-shaped tables.
- **When locked:** Saturday 2026-04-25, late afternoon.
- **Why:** Permission to *use* the external benchmark data once does not equal permission to *publish* it. Public repo + work-side data risk fingerprinting (Cannes / Morning Consult / Pathmatics methodology, agency, client). Time and demo-coherence concerns also remain even with permission. The split lets the team show vision without overpromising or polluting the public surface.
